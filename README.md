...
キー操作: `⌘S`/`Ctrl+S` 保存 ・ `N` 付箋モード ・ `Esc` 付箋モード解除／選択解除 ・ `⌘Z`/`Ctrl+Z` 元に戻す ・ `⇧⌘Z`/`Ctrl+Y` やり直す

元に戻せるのは直前 5 手まで（コメントの追加・削除・返信・取り込み、本文の編集）。1 つのコメントへの続けた入力は 1 手として扱う。入力欄にカーソルがあるときの `⌘Z` は、その欄の文字だけを戻すブラウザ標準の動作になる。
